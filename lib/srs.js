...
// ログアウト時にローカル SRS データを消去する（クロスユーザー汚染防止）
...
// 改修3（2026-06-16）: 最小チェックポイントは「翌日（dateStr+1）」を明示保証する。
//   dateStr = today の場合、+1 = tomorrow なので当日のセッション内には一切再表示されない。
export function recordReviewMiss(srsState, senseId, dateStr) {
  const records = { ...(srsState.records || {}) };
  const prev = records[senseId];
  if (!prev) return srsState;

  // 既存の残チェックポイントを破棄し、再ミス日の翌日を起点に全サイクルを張り直す。
  // +1 を最小にすることで、当日中（同セッション/別セッション問わず）に再出題されない。
  const tomorrowStr = addDays(dateStr, 1); // 翌日（最早出題日）
  const newCheckpoints = [
    tomorrowStr,                // dateStr + 1（翌日）
    addDays(dateStr, 3),        // dateStr + 3
    addDays(dateStr, 7),        // dateStr + 7
    addDays(dateStr, 30),       // dateStr + 30
  ].sort();
