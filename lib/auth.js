...
export async function signInWithEmail(email) {
  ...
  // emailRedirectTo に「今いるURL」を渡すと、dev プレビューでも本番でも
  // マジックリンクが要求元オリジンへ正しく戻る（Redirect URLs 許可リストに dev/本番/localhost 登録済）。
  // 未指定だと Site URL 固定先（=dev）に飛んでしまう。
...
// Auth 状態変化のリスナー登録
// callback は (event: string, session: Session | null) => void を受け取る
// event には supabase-js が渡す INITIAL_SESSION / SIGNED_IN / SIGNED_OUT 等が入る。
// 呼び出し側でイベント名を見て SIGNED_OUT（明示ログアウト）か否かを区別できる。
