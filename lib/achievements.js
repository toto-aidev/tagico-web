 ...
 * 返り値:
 *   {
 *     earnedIdx: number,   // 獲得済みティアの最後のインデックス（0-based）。-1 = 1つも未獲得
 *     nextTier:  object|null,  // 進行中ティア（earnedIdx+1 番）。null = 全コンプリート
...
export function getGroupStatus(group, appState) {
  ...
  if (group.id === 'collector') {
    ...
    for (let i = 0; i < tiers.length; i++) {
      ...
      if (val >= tier.threshold) {
        ...
        break; // 連続して獲得が前提（前のティアが取れていないと次は取れない想定）
      }
    }
    const nextIdx = earnedIdx + 1;
    if (nextIdx >= tiers.length) {
      // 全コンプリート
      return { earnedIdx, nextTier: null, nextValue: tiers[tiers.length - 1].threshold, nextThreshold: tiers[tiers.length - 1].threshold };
    }
    const nextTier = tiers[nextIdx];
    const nextValue = nextTier.field === 'bookmarks'
      ? (appState.bookmarks || []).length
      : (appState.savedSenses || []).length;
    return { earnedIdx, nextTier, nextValue, nextThreshold: nextTier.threshold };
  }
  ...
  const nextIdx = earnedIdx + 1;
  if (nextIdx >= tiers.length) {
    return { earnedIdx, nextTier: null, nextValue: value, nextThreshold: tiers[tiers.length - 1].threshold };
  }
  return {
    earnedIdx,
